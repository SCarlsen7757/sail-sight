...
CARTO provides theme-dependent basemaps; the current watermark/provider problem is tracked in [issue #26](https://github.com/SCarlsen7757/sail-sight/issues/26). The UI offers OpenSeaMap, but the current CSP omits its tile host, so that overlay can be blocked. Documentation screenshots use stub tiles and do not validate either external service.
